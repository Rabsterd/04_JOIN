...
-- ANSI
-- 연결에 사용할 컬럼명이 같은 경우 UNSING(컬럼명)을 사용함
...
WHERE E.JOB_CODE = J.JOB CODE;

-- INNER JOIN(내부 조인) 시 문제점!
--> 연결에 사용된 컬럼의 값이 일치하지 않으면 조회 결과에 포함
되지 않는다
...
-- 2) RIGHT [OUTER] JOIN : 합치기에 사용한 두 테이블 중 오른편에 기술된
-- 테이블의 컬럼수를 기준으로 JOIN
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE RIGHT JOIN DEPARTMENT
ON (DEPT_CODE = DEPT_ID); -- 24행 (마케팅부, 국내영업부, 해외영업부 3부 포함)

-- 3) FULL [OUTER] JOIN : 합치기에 사용한 두 테이블이 가진 모든 행을 결과에 포함
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE FULL JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); --26행

-- 오라클 구문( FULL [OUTER] JOIN 없음 )
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+); -- LEFT JOIN 23행
-- 반대쪽 테이블 컬럼에 (+) 기호를 작성해야 한다

SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE(+) = DEPT_ID; -- RIGHT JOIN 24행
...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
CROSS JOIN DEPARTMENT; -- 207행 == 23(EMP_NAME) * 9(DEPT_TITLE)
...
--> 다중 조인시 앞에서 조인된 결과에 새로운 테이블 내용을 조인

-- ORA-0094 : "컬럼명" : 부적합한 식별자 -> JOIN 순서 오류

-- 오라클
SELECT EMP_NAME, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE, DEPARTMENT, LOCATION
WHERE DEPT_CODE = DEPT_ID -- EMPLOYEE + DEPARTMENT 조인
AND LOCATION_ID = LOCAL_CODE; -- (EMPLOYEE + DEPARTMENT) + LOCATON 조인
